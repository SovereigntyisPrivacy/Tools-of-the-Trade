...
export const saveVideoWallpaper = async (blob) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = tx.objectStore(STORE_NAME).put(blob, 'custom_video');
    
    // MAGIC BULLET: Wait for hardware disk write to finish before resolving
    tx.oncomplete = () => {
...
export default function LiveWallpaper() {
  ...
  return (
    <video
      ...
      style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', objectFit: 'cover', zIndex: -1, pointerEvents: 'none', opacity: 0.5 }}
    />
  );
}
