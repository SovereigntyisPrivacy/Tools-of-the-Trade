...
function TimesheetUI() {
  ...
  const [expanded, setExpanded] = useState({}); // Controls collapsible grid
  ...
  // --- PERSONAL SHIFT TRACKER (Employee Tab) ---
                  ...
                  return (
                    <div key={emp.id} style={{ ...cardStyle, padding: '15px', borderLeft: emp.type === '1099' ? '4px solid #f59e0b' : '4px solid #a855f7' }}>
                      ...
                      {expanded[emp.id] && (
                          ...
                          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => {
                            const s = shifts[day] || {in: '', out: '', flag: ''}; const hrs = calcShiftHrs(s.in, s.out);
                            return (
                              <div key={day} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                                ...
                                }} style={{ background: s.flag === 'late' ? 'rgba(245,158,11,0.2)' : s.flag === 'absent' ? 'rgba(239,68,68,0.2)' : '#000', border: '1px solid #333', borderRadius: '6px', padding: '8px 10px', fontSize: '1.1em' }}>{s.flag === 'late' ? '⏰' : s.flag === 'absent' ? '❌' : '✓'}</button>
                              </div>
                            )
                          })}
                          </div>
            ...
            {mgrTab === 'Report' && (
              <div style={cardStyle}>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <div style={{ background: '#111', border: '1px solid #222', borderRadius: '8px', padding: '10px', marginBottom: '15px' }}>
                      {w2Roster.map(emp => {
                        const shifts = activeWeek.shifts[emp.id] || getEmptyShifts();
                      ...
                      <div style={{ background: '#000', padding: '15px', borderRadius: '8px', border: '1px solid #333' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '10px', borderTop: '1px dashed #333' }}>
                          <strong style={{ color: '#fff' }}>Est. Payroll Liability:</strong><strong style={{ color: '#a855f7', fontSize: '1.5em' }}>${fleetPay.toFixed(2)}</strong>
                        </div>
                      </div>
                    </>
                  );
