...
function SurvivalLibrary() {
  ...
  const handleSearch = async (e) => {
    ...
    setSearched(true);
    try {
      // Strict Search: Forces the archive to look for survival, medical, and field manual keywords
      const strictQuery = `(${query}) AND (title:survival OR title:"first aid" OR title:bushcraft OR title:foraging OR title:manual OR subject:survival OR subject:"first aid" OR subject:wilderness)`;
      ...
      console.error("Library DB Error:", error);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="view-wrapper pb-safe">
      ...
      <div className="calc-content" style={{ padding: '20px', overflowY: 'auto', height: '100%', paddingBottom: '120px' }}>
        <p style={{ color: '#00cc66', fontWeight: 'bold', marginBottom: '15px' }}>📚 Field Manuals & Medical Guides</p>
        
        <form onSubmit={handleSearch} style={{ marginBottom: '25px' }}>
          <div style={{ display: 'flex', gap: '10px' }}>
            <input 
              type="text" 
              placeholder="e.g., 'First Aid', 'Water Purification', 'FM 21-76'..." 
              value={query}
        ...
        {loading ? (
          <div style={{ color: '#aaa', textAlign: 'center', marginTop: '20px' }}>Querying survival archives...</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {results.map((item, idx) => (
              <div key={idx} style={{ background: 'rgba(20, 20, 20, 0.8)', border: '1px solid #333', borderLeft: '4px solid #00cc66', borderRadius: '8px', padding: '15px' }}>
                <div style={{ fontWeight: 'bold', color: '#fff', fontSize: '1.1em', marginBottom: '5px' }}>{item.title}</div>
                <div style={{ color: '#888', fontSize: '0.85em', marginBottom: '5px' }}><span style={{ color: '#aaa' }}>Author/Publisher:</span> {item.creator ? item.creator : 'Unknown'}</div>
                ...
                <button 
                  ...
                  style={{ width: '100%', padding: '12px', background: 'rgba(0, 204, 102, 0.1)', color: '#00cc66', border: '1px solid #00cc66', borderRadius: '5px', fontSize: '1em', fontWeight: 'bold', cursor: 'pointer' }}
                >
            ...
            {searched && results.length === 0 && !loading && (
              <div style={{ color: '#ff4444', textAlign: 'center', marginTop: '20px' }}>No manuals found. Try a broader search term like "First Aid" or "Bushcraft".</div>
            )}
          </div>
        )}
      </div>
...
export default SurvivalLibrary;
