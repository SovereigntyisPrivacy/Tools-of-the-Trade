    ...
    <div className="view-wrapper" style={{ background: '#0a0a0a', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '20px', flex: 1, overflowY: 'auto', paddingBottom: '100px' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
            ...
            <input type="number" value={bill} onChange={e => setBill(e.target.value)} placeholder="0.00" style={{ ...inputStyle, fontSize: '2em', color: '#00cc66', borderColor: '#00cc66' }} />
        </div>
        ...
        <div style={{ display: 'flex', gap: '15px', marginBottom: '20px' }}>
            <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    ...
                    <select onChange={(e) => setTax(e.target.value)} style={{ background: 'transparent', color: '#888', border: 'none', outline: 'none', fontSize: '0.85em', width: '60px', padding: 0 }}>
                        <option value="8.7">AZ 📍</option>
                        <option value="7.25">CA</option>
                        <option value="6.25">TX</option>
                        <option value="6.0">FL</option>
                        <option value="4.0">NY</option>
        ...
        <div style={{ display: 'flex', gap: '8px', marginTop: '6px', marginBottom: '25px' }}>
            {[15, 18, 20, 25].map(pct => (
                <button key={pct} onClick={() => setTip(pct.toString())} style={{ flex: 1, padding: '12px 0', borderRadius: '8px', border: 'none', fontWeight: 'bold', fontSize: '1.1em', background: tip === pct.toString() ? '#00cc66' : '#222', color: tip === pct.toString() ? '#000' : '#fff' }}>
                    {pct}%
                </button>
            ))}
        </div>
