...
export default function MasterCalendar() {
  ...
  const getOccurrencesInMonth = (baseDateStr, frequency, year, month) => {
    ...
    else if (frequency === 'Bi-Weekly') interval = 14;
    else if (frequency === 'Monthly') {
      const targetDay = baseDate.getDate();
      const daysInTargetMonth = new Date(year, month + 1, 0).getDate();
      const actualDay = targetDay > daysInTargetMonth ? daysInTargetMonth : targetDay;
      const occurrence = new Date(year, month, actualDay);
      if (occurrence >= baseDate) return [toYMD(occurrence)];
      return [];
    }
    
    if (interval > 0) {
      let stepDate = new Date(baseDate);
      if (stepDate < viewStart) {
        const msPerDay = 1000 * 60 * 60 * 24;
        const diffDays = Math.ceil((viewStart - baseDate) / msPerDay);
        const stepsNeeded = Math.ceil(diffDays / interval);
        stepDate.setDate(baseDate.getDate() + (stepsNeeded * interval));
      }
      let safety = 0;
      while (stepDate <= viewEnd && safety < 100) {
        if (stepDate >= baseDate && stepDate >= viewStart) occurrences.push(toYMD(stepDate));
    ...
    const scanModules = () => {
      ...
      const processRecurringData = (items, color, icon, type) => {
        items.forEach(item => {
          ...
          if (!occurrences.length && rawDateStr.startsWith(`${currentYear}-${String(currentMonth+1).padStart(2, '0')}`)) {
             occurrences.push(rawDateStr);
          }
      ...
      // 4. Contractor & Shifts
      getSafe('hos_dotArchives').forEach(d => { if (d.date) globalEvents.push({ id: `dot_${Date.now()}`, date: normalizeDate(d.date), title: `HOS Logged`, type: 'DOT Shift', icon: '🚚', color: '#f59e0b' }); });
      getSafe('hos_invoiceArchives').forEach(i => { if (i.date) globalEvents.push({ id: `inv_${Date.now()}`, date: normalizeDate(i.date), title: `Invoice`, type: 'Contractor Job', icon: '🛠️', amount: i.total, color: '#f59e0b' }); });
      [...getSafe('fleet_shifts'), ...getSafe('my_personal_shifts')].forEach(s => { if (s.date) globalEvents.push({ id: `shft_${Date.now()}`, date: normalizeDate(s.date), title: `Shift Logged`, type: 'Work', icon: '🏢', color: '#06b6d4' }); });
    ...
    <div className="view-wrapper" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff' }}>
      ...
      <div style={{ background: '#111', margin: '15px 10px 100px 10px', borderRadius: '12px', padding: '20px', borderTop: '4px solid #a855f7' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #333', paddingBottom: '15px', marginBottom: '20px' }}>
          ...
          <span style={{ color: '#a855f7', fontWeight: 'bold', fontSize: '1.1em' }}>{displayDateText}</span>
        </div>
        
        {selectedDayEvents.length === 0 ? <p style={{ color: '#666', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>No events logged.</p> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
            {selectedDayEvents.map((e, idx) => (
              <div key={idx} style={{ display: 'flex', alignItems: 'center', background: '#0a0a0a', padding: '15px', borderRadius: '10px', borderLeft: `4px solid ${e.color}`, border: '1px solid #222', opacity: e.isPaid ? 0.6 : 1 }}>
                <div style={{ fontSize: '1.8em', marginRight: '15px' }}>{e.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ color: e.color, fontSize: '0.75em', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '1px' }}>{e.type}{e.isPaid && ' ✅'}</div>
                  <div style={{ color: '#fff', fontWeight: 'bold', fontSize: '1.1em', textDecoration: e.isPaid ? 'line-through' : 'none' }}>{e.title}</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
                  {e.amount && <div style={{ color: '#fff', fontSize: '1.1em', fontWeight: 'bold', textDecoration: e.isPaid ? 'line-through' : 'none' }}>${parseFloat(e.amount).toFixed(2)}</div>}
                  {e.isCustom && <button onClick={() => deleteCustomEvent(e.id)} style={{ background: 'rgba(239, 68, 68, 0.2)', color: '#ef4444', border: 'none', padding: '4px 10px', borderRadius: '4px', fontWeight: 'bold', fontSize: '0.8em' }}>DELETE</button>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ borderTop: '1px solid #222', paddingTop: '20px' }}>
          ...
          <button onClick={handleAddCustomEvent} style={{ width: '100%', background: 'rgba(168, 85, 247, 0.1)', color: '#a855f7', border: '1px solid #a855f7', padding: '15px', borderRadius: '8px', fontWeight: 'bold', marginTop: '15px', fontSize: '1.1em' }}>+ Add to Agenda</button>
        </div>
      </div>
    </div>
  );
