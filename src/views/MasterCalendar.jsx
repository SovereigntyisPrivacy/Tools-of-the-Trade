...
export default function MasterCalendar() {
  ...
  const normalizeDate = (dStr) => {
    ...
    return dStr.substring(0, 10); // Standardize to YYYY-MM-DD
  };
  ...
  useEffect(() => {
    const scanModules = () => {
      ...
      const shifts = [...getSafe('fleet_shifts'), ...getSafe('my_personal_shifts')];
      const manualSyncs = getSafe('tot_calendar_events'); // Explicit Sync Buttons

      // 1. Explicit Sync Buttons (From Budget Engine, etc.)
      manualSyncs.forEach(e => {
        if(e.date) globalEvents.push({ id: e.id, date: normalizeDate(e.date), title: e.title, amount: e.amount, color: '#3b82f6' });
      });

      // 2. Budget Recurrences
      const processRecurring = (items, type, color) => {
        items.forEach(item => {
          ...
          if (!item.frequency || item.frequency === 'None') {
            globalEvents.push({ id: `${type}_${Date.now()}`, date: rawDate, title: `${type}: ${name}`, amount: item.amount || item.cost, color });
          } else {
            // Simplified rendering for recurring just on the base date for safety
            globalEvents.push({ id: `${type}_${Date.now()}`, date: rawDate, title: `${type}: ${name} 🔁`, amount: item.amount || item.cost, color });
          }
        });
      };
      ...
      assets.forEach(a => {
        if (a.warranty && a.warranty !== 'None' && a.warranty !== 'Lifetime' && a.date) {
           const nDate = normalizeDate(a.date);
           if(nDate) globalEvents.push({ id: `ast_${Date.now()}`, date: nDate, title: `Warranty: ${a.name || 'Asset'}`, color: '#a855f7' });
        }
      });

      // 4. Chronos & Shifts
      getSafe('hos_dotArchives').forEach(d => { if (d.date) globalEvents.push({ id: `dot_${Date.now()}`, date: normalizeDate(d.date), title: `DOT Shift`, color: '#f59e0b' }); });
      getSafe('hos_invoiceArchives').forEach(i => { if (i.date) globalEvents.push({ id: `inv_${Date.now()}`, date: normalizeDate(i.date), title: `Job Invoice`, amount: i.total, color: '#f59e0b' }); });
      shifts.forEach(s => { if (s.date) globalEvents.push({ id: `shft_${Date.now()}`, date: normalizeDate(s.date), title: `Work Shift`, color: '#06b6d4' }); });
  ...
  const handleAddCustomEvent = () => {
    ...
    const color = customType === 'Priority' ? '#f59e0b' : customType === 'Urgent' ? '#ef4444' : customType === 'Task' ? '#3b82f6' : '#fff';
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      cells.push(
        <div key={day} onClick={() => handleDayClick(day)} style={{ background: '#111', border: isSelected ? '2px solid #a855f7' : '1px solid #222', minHeight: '60px', padding: '5px', display: 'flex', flexDirection: 'column', cursor: 'pointer' }}>
          <span style={{ color: isSelected ? '#a855f7' : '#fff', fontWeight: 'bold', fontSize: '0.9em', alignSelf: 'center', marginBottom: '5px' }}>{day}</span>
  ...
  return (
    <div className="view-wrapper" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', background: '#111', borderBottom: '1px solid #222', borderRadius: '12px 12px 0 0', margin: '10px 10px 0 10px' }}>
        ...
        <h2 style={{ margin: 0, color: '#ef4444', fontSize: '1.2em', textTransform: 'uppercase' }}>{monthNames[currentMonth]} {currentYear}</h2>
      ...
      <div style={{ background: '#111', margin: '15px 10px 100px 10px', borderRadius: '12px', padding: '20px', borderTop: '4px solid #a855f7' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #333', paddingBottom: '10px', marginBottom: '15px' }}>
          <h3 style={{ color: '#ef4444', margin: 0 }}>Agenda:</h3><span style={{ color: '#a855f7', fontWeight: 'bold' }}>{displayDateText}</span>
        </div>
        {selectedDayEvents.length === 0 ? <p style={{ color: '#666', fontStyle: 'italic', textAlign: 'center' }}>No events found.</p> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
            {selectedDayEvents.map((e, idx) => (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', background: '#0a0a0a', padding: '12px', borderRadius: '8px', borderLeft: `3px solid ${e.color}` }}>
                <span style={{ color: e.color, fontWeight: 'bold' }}>{e.title}</span>
                <div style={{ display: 'flex', gap: '10px' }}>
                  {e.amount && <span style={{ color: e.color, fontWeight: 'bold' }}>${parseFloat(e.amount).toFixed(2)}</span>}
                  {e.isCustom && <button onClick={() => deleteCustomEvent(e.id)} style={{ background: 'transparent', color: '#ef4444', border: 'none', fontWeight: 'bold' }}>×</button>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', gap: '10px', borderTop: '1px dashed #333', paddingTop: '20px' }}>
          ...
          <select value={customType} onChange={(e) => setCustomType(e.target.value)} style={{ ...inputStyle, flex: 1, margin: 0 }}><option>Normal</option><option>Task</option><option>Priority</option><option>Urgent</option></select>
        </div>
        <button onClick={handleAddCustomEvent} style={{ width: '100%', background: 'rgba(168, 85, 247, 0.1)', color: '#a855f7', border: '1px solid #a855f7', padding: '15px', borderRadius: '8px', fontWeight: 'bold', marginTop: '15px' }}>+ Log to Agenda</button>
      </div>
    </div>
  );
}
