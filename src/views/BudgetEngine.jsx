...
export default function BudgetEngine() {
  ...
  const handleAddIncome = () => {
    ...
    setIncomeName(''); setIncomeAmount(''); setIncomeNote(''); setActiveTab('ledger');
  };
  ...
  const handleAddBill = () => {
    ...
    setBillName(''); setBillCost(''); setBillNote(''); setActiveTab('ledger');
  };
    ...
    <div style={{ marginTop: '20px' }}>
      ...
      {sortedBills.map(bill => (
        <div key={bill.id} style={{ ...glassCard, borderLeft: '4px solid #ef4444', padding: '15px', opacity: bill.isPaid ? 0.6 : 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
            <div>
              <h3 style={{ color: '#a855f7', margin: '0 0 5px 0', textDecoration: bill.isPaid ? 'line-through' : 'none' }}>{bill.name}</h3>
              <span style={{ background: '#111', color: '#ef4444', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem' }}>{bill.frequency}</span>
            </div>
            <div style={{ color: '#ef4444', fontSize: '1.3rem', fontWeight: 'bold', textDecoration: bill.isPaid ? 'line-through' : 'none' }}>${bill.cost.toFixed(2)}</div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
            ...
            <span style={{ background: bill.isPaid ? '#10b981' : '#ef4444', color: bill.isPaid ? '#000' : '#fff', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 'bold' }}>{bill.isPaid ? 'PAID ✅' : 'UNPAID'}</span>
          </div>
      ...
      <div style={{ padding: '0 15px 100px 15px', overflowY: 'auto' }}>
        ...
        {activeTab === 'add-inc' && (
          <div style={{ ...glassCard, borderTop: '4px solid #10b981', marginTop: '20px' }}>
            ...
            <input type="text" placeholder="Notes (Optional)..." value={incomeNote} onChange={e=>setIncomeNote(e.target.value)} style={inputStyle} />
        ...
        {activeTab === 'add-bill' && (
          <div style={{ ...glassCard, borderTop: '4px solid #ef4444', marginTop: '20px' }}>
            ...
            <input type="text" placeholder="Notes (Optional)..." value={billNote} onChange={e=>setBillNote(e.target.value)} style={inputStyle} />
