...
export default function MyShiftTracker() {
  ...
  const [hourlyRate, setHourlyRate] = useState(() => parseFloat(localStorage.getItem('tot_shift_rate')) || 16.00);
  const [taxProfile, setTaxProfile] = useState(() => localStorage.getItem('tot_shift_tax_prof') || 'W2 Employee');
  const [stateTax, setStateTax] = useState(() => parseFloat(localStorage.getItem('tot_shift_state_tax')) || 2.5);
  const [fedTax, setFedTax] = useState(() => parseFloat(localStorage.getItem('tot_shift_fed_tax')) || 10.0);
  ...
  const [shifts, setShifts] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : {
      SUN: [{ start: '', end: '', note: '' }], MON: [{ start: '', end: '', note: '' }],
      TUE: [{ start: '', end: '', note: '' }], WED: [{ start: '', end: '', note: '' }],
      THU: [{ start: '', end: '', note: '' }], FRI: [{ start: '', end: '', note: '' }],
      SAT: [{ start: '', end: '', note: '' }]
    };
  });

  useEffect(() => {
    localStorage.setItem('tot_shift_rate', hourlyRate); localStorage.setItem('tot_shift_tax_prof', taxProfile);
    localStorage.setItem('tot_shift_state_tax', stateTax); localStorage.setItem('tot_shift_fed_tax', fedTax);
    localStorage.setItem('tot_shift_archives', JSON.stringify(archives)); localStorage.setItem('tot_shift_shifts', JSON.stringify(shifts));
  }, [hourlyRate, taxProfile, stateTax, fedTax, archives, shifts]);

  const stateTaxOptions = [
    { label: 'Arizona (2.5%)', value: 2.5 }, { label: 'California (9.3%)', value: 9.3 },
    { label: 'Texas (0%)', value: 0.0 }, { label: 'Nevada (0%)', value: 0.0 }, { label: 'Custom Flat (5.0%)', value: 5.0 }
  ];

  const LaborGuides = [
    { title: "Wage & Overtime Rights (FLSA)", content: "Under the Fair Labor Standards Act (FLSA), federal law mandates that covered nonexempt employees must receive overtime pay for hours worked over 40 per workweek at a rate not less than time and one-half their regular rates of pay. Employers cannot force you to work 'off the clock' or average your hours across two weeks to avoid overtime.", contact: "Wage and Hour Division (WHD): 1-866-487-9243" },
    { title: "Right to Discuss Pay (NLRB)", content: "Under the National Labor Relations Act (NLRA), you have the federally protected right to discuss your wages, hours, and working conditions with your coworkers. If an employer has a policy forbidding you from discussing pay, or if they fire/retaliate against you for doing so, they are breaking federal law.", contact: "National Labor Relations Board: 1-844-762-6572" },
    { title: "Workplace Safety (OSHA)", content: "You have the right to a safe workplace. The Occupational Safety and Health Act strictly prohibits employers from retaliating against employees who raise safety or health concerns. You have the right to refuse dangerous work if you genuinely believe it exposes you to an imminent danger of death or serious injury, and your employer cannot fire you for it.", contact: "OSHA Emergency Line: 1-800-321-6742" },
    { title: "1099 vs W2 Misclassification", content: "Many companies misclassify workers as 1099 Independent Contractors to avoid paying employer taxes, overtime, and benefits. If your employer dictates exactly when, where, and how you do your job, you are legally a W2 employee. Misclassification is tax fraud by the employer." }
  ];
  ...
  const addShift = (day) => setShifts(prev => ({ ...prev, [day]: [...prev[day], { start: '', end: '', note: '' }] }));
  const removeShift = (day, index) => {
    const newShifts = shifts[day].filter((_, i) => i !== index);
    if (newShifts.length === 0) newShifts.push({ start: '', end: '', note: '' });
    setShifts(prev => ({ ...prev, [day]: newShifts }));
  };
  const clearWeek = () => setShifts({ SUN: [{ start: '', end: '', note: '' }], MON: [{ start: '', end: '', note: '' }], TUE: [{ start: '', end: '', note: '' }], WED: [{ start: '', end: '', note: '' }], THU: [{ start: '', end: '', note: '' }], FRI: [{ start: '', end: '', note: '' }], SAT: [{ start: '', end: '', note: '' }] });

  // --- THE SMART PUNCH ENGINE ---
  const handleSmartPunch = () => {
    const now = new Date();
    const today = daysOfWeek[now.getDay()];
    const yesterday = daysOfWeek[(now.getDay() + 6) % 7];
    const timeStr = now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });

    // 1. Check yesterday for an unclosed shift (Critical for overnight workers)
    let yShifts = [...shifts[yesterday]];
    let openYIndex = yShifts.findIndex(s => s.start && !s.end);
    if (openYIndex >= 0) {
      yShifts[openYIndex].end = timeStr;
      setShifts(prev => ({ ...prev, [yesterday]: yShifts }));
      return;
    }

    // 2. Otherwise, check today
    let tShifts = [...shifts[today]];
    let openTIndex = tShifts.findIndex(s => s.start && !s.end);
    if (openTIndex >= 0) {
      tShifts[openTIndex].end = timeStr;
    } else {
      // Find empty slot or create new
      if (tShifts.length > 0 && !tShifts[tShifts.length - 1].start && !tShifts[tShifts.length - 1].end) {
        tShifts[tShifts.length - 1].start = timeStr;
      } else {
        tShifts.push({ start: timeStr, end: '', note: '' });
      }
    }
    setShifts(prev => ({ ...prev, [today]: tShifts }));
  };
  ...
  daysOfWeek.forEach((day) => {
    ...
    shifts[day].forEach(shift => {
      if (shift.start && !shift.end) isCurrentlyPunchedIn = true;
    ...
    if (dayHours > 0) dailyBreakdown.push({ day, hours: dayHours.toFixed(2), shifts: shiftStrs });
  });
  ...
  const ssTax = isW2 ? grossPay * 0.062 : grossPay * 0.124; // FICA Social Security
  const medTax = isW2 ? grossPay * 0.0145 : grossPay * 0.029; // FICA Medicare
  ...
  return (
    <div className="view-wrapper" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff' }}>
      ...
      <div style={{ padding: '0 15px 100px 15px', overflowY: 'auto' }}>
        {activeTab === 'tracker' && (
          <>
            {/* THE SMART PUNCH BUTTON */}
            <button onClick={handleSmartPunch} style={{ width: '100%', background: isCurrentlyPunchedIn ? '#ef4444' : '#10b981', color: isCurrentlyPunchedIn ? '#fff' : '#000', border: 'none', padding: '20px', borderRadius: '12px', fontSize: '1.5rem', fontWeight: '900', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '20px', boxShadow: isCurrentlyPunchedIn ? '0 0 20px rgba(239, 68, 68, 0.4)' : '0 0 20px rgba(16, 185, 129, 0.4)' }}>
              {isCurrentlyPunchedIn ? 'CLOCK OUT' : 'CLOCK IN'}
            </button>
            ...
            <div style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '4px solid #10b981' }}>
              ...
              <div style={{ width: '45%', textAlign: 'right' }}>
                ...
                <div style={{ color: '#10b981', fontSize: '2em', fontWeight: 'bold' }}>{totalHours.toFixed(2)}</div>
              </div>
            </div>
                  ...
                  {shifts[day].map((shift, idx) => (
                    <div key={idx} style={{ marginBottom: '12px', background: 'rgba(255,255,255,0.03)', padding: '10px', borderRadius: '8px', border: '1px solid #333' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                        ...
                        <button onClick={() => removeShift(day, idx)} style={{ background: 'transparent', color: '#ef4444', border: 'none', padding: '8px', fontSize: '1.2rem', margin: 0 }}>X</button>
                      </div>
                      <input type="text" placeholder="Shift notes (job site, delays, coverage)..." value={shift.note || ''} onChange={(e) => handleShiftChange(day, idx, 'note', e.target.value)} style={{ ...inputStyle, padding: '8px', margin: 0, fontSize: '0.9em', color: '#aaa', width: '100%', boxSizing: 'border-box' }} />
                    </div>
                  ))}
                </div>
              ))}
            ...
            <div style={{ ...cardStyle, borderLeft: '4px solid #10b981' }}>
              ...
              <button onClick={() => setShowReport(true)} style={{ width: '100%', background: '#10b981', color: '#000', border: 'none', padding: '15px', borderRadius: '8px', fontWeight: 'bold', marginTop: '25px', fontSize: '1.05em' }}>📄 VIEW FULL REPORT</button>
            </div>
          </>
        )}
        ...
        {showReport && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: '#000', zIndex: 100, padding: '20px', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
              ...
              {dailyBreakdown.length === 0 ? (
                ...
                dailyBreakdown.map((log) => (
                  <div key={log.day} style={{ marginBottom: '10px', color: '#ccc' }}>
                    ...
                    {log.shifts.map((s, i) => <div key={i} style={{ fontSize: '0.85em', color: '#888', textAlign: 'right' }}>{s}</div>)}
                  </div>
                ))
              )}
            ...
            <div style={{ background: '#111', padding: '20px', borderRadius: '12px', border: '1px solid #333', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '15px', borderTop: '2px solid #333', paddingTop: '15px', fontSize: '1.3em', fontWeight: 'bold' }}>
